...
var decimalToBinaryReady = function() {
	...
	$("#convert").click(function(){ 
		...
		if (!isNaN( $("#decimalValue").val() ) && $("#decimalValue").val() >= base ) {
			...
			switch(count) {
			case 3:
					$('#box0').css('margin-left', resultDivWidth / 6.9);
					break;
			case 2:
					$('#box0').css('margin-left', resultDivWidth / 4.2);
					break;
			case 1:
					$('#box0').css('margin-left', resultDivWidth / 2.97);
					break;
			}
			$("#remaindersRow").removeClass("hidden");
			for (var i = binaryValues.length - 1,j=0; i >= 0; i--,j++) {
				$("#storeRemainder" + j).text( binaryValues[i] );
			}
			introguide.nextStep();
		} else {
			$("#convert").addClass("disabled").addClass("opacity40");
...
function introGuide() {
	...
	introguide.onbeforechange(function(targetElement) {
		...
		if (elementId == "inputDiv") {
			...
			typingContent = '<span id="inputText">Enter a decimal value and click the \"Convert to Binary\" button to see how the Decimal to Binary' + "" 
...
function dividendAnimation(index) {
	...
	if (index == 0) {
		...
		message = 'Now consider the previous quotient.';
	}
	setTimeout(function () {
		typingId = '.introjs-tooltiptext';
...
function dividerAnimation(index) {
		...
		typingId = '.introjs-tooltiptext';
		typingContent = 'In <span class="ct-code-b-yellow">Binary System</span>, since the base is' + ""
		...
		TweenLite.from("#secondValue" + index, 1, {top: topLength1, left: leftLength1, onComplete: function(){
			...
			TweenMax.fromTo("#firstValue" + index + ",#secondValue" + index + ",#mulOperator" + index, 1 ,{delay: 2, rotationX: 0}, {rotationX: 180, opacity: 0, onComplete: function(){
				...
				$('#productSpan' + index).fadeTo(200, 1,function(){
					$("#remainder" + index).css('border-top','0px solid black');
					if (index > 1) {
							remainderAnimationForFirstTime(index);
					} else {
						$('.introjs-nextbutton').css("visibility", "visible");
		...
		if (index == 0) {
			...
			typingCallbackFunction = function () {
				...
				TweenLite.from($("#storeRemainder" + index), 2, {top: topLength, left:leftLength, onComplete: function() {
					...
					$('#quotientText').empty();
					typingId = '#outputText';
					typingContent = 'This is the binary representation of ' + $("#decimalValue").val() + '.';
					typingCallbackFunction = function () {
						$('.introjs-nextbutton').show();
					}
					typing(typingId, typingContent, typingCallbackFunction);
...
function binaryValueAnimation() {
	setTimeout(function(){
		...
		typingContent = "Binary representation of the given number <span style='font-family: monospace'>" + $("#decimalValue").val() + "</span>" + ''
