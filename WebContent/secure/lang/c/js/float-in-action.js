...
var floatInActionReady = function() {
	...
	introcode.onbeforechange(function(targetElement) {
		...
		case "mainDiv":
			$('#mainnumDiv').hide();  
			$('.first-heading').text('float data type');
			break;
		case "mainnumDiv":
			$('#mainnumDiv').show(); 
			$("#int1Div").fadeTo(10, 0);
			$("#divisionBtn,#int2Div,#intpart,.introjs-prevbutton").hide();
			$("#label,#tableDiv, #desc").remove()
			break;
		case "intpart":
			$('#step0, #one,#tableDiv,#label, #desc, .user-btn').remove();
			$("#intpart").show();
			$("#int3Div,#int4Div").css({'opacity':'0'});
			$('#bDiv').css({'opacity':'0'});
			$("#int2Div,#divisionBtn,.introjs-prevbutton").hide();
			$("#int1Div").fadeTo(10, 0);
			break;
		case "fractpart":
			$("#cpyMulFraction4,#cpyMulFraction, #mul2, #binVal1, #label2, #one").remove();
			$("#int3Div,#int4Div").css({'opacity':'0'});
			$("#floatText, #floatText1, #floatText2").remove();
			$("#finalDivtext").empty();
			divisionStep = 5;
			break;
		case "finalDivtext":
			$("#floatText, #floatText1, #floatText2 ").remove()
			$("#finalDivtext").empty();
			$("#floatanimation, #deduceDiv").remove();
			break;
		case "shiftdotDiv":
			$("#floatanimation, #deduceDiv").remove();
			$('#signval, #expval, #manval').text("");
			$('#lastvalDiv,#lastDiv').css({'opacity':'0'});
			$("#lastbitDiv").css("opacity", "0");
			break;
		case "finalDiv":
			$('#signval, #expval, #manval').text("");
			$('#lastvalDiv,#lastDiv').css({'opacity':'0'});
	...
	introcode.onafterchange(function(targetElement) {
		...
		case "mainDiv":
			if (introcode._direction == "backward") {
				$("#mainDiv").removeClass('visibility-hidden'); 
				$('.first-heading').text('float data type');
				$('#floatdec').css({'opacity':'0'});
				$('#floatVar, #inputDiV').fadeTo(10,0);
				$('#addDiv, #floatBox, .byte-div').fadeTo(10,0);
				animationbtn2(); 
			} else {
				$('.introjs-nextbutton').hide();
				$("#mainDiv").removeClass('visibility-hidden'); 
				typingContent = '<ul><li><span class = color-green>float data</span> represents real numbers with a '+
						'decimal point.</li><li>It occupies <span class = color-green>4 bytes(32 bits)</span> '+
						'in the memory.</li><span id = "typing11"></span>';
				typing('#typing1',typingContent,function() {
					typingContent = '<li><span class = color-green>float </span> stores in <span class = color-green>'+
							'<b>IEEE 754</b> standard</span> format.</li><span id = "typing111"></span></ul>';
					typing('#typing11',typingContent,function() {
						typingContent = '<ul><li><span class = color-green>IEEE 754 standard</span> specifies '+
								'the memory given for a float variable as </li></ul>';
						typing('#typing111',typingContent,function() {
							...
							$('#first1Div,#floatnumDiv,#floatsignDiv,#firstDiv').css({'opacity' : '1'});
							$("#box0").effect( "highlight",{color: 'yellow'}, 500,function() {
								$("#int12Div ,#step2 , #step0,  #floatDecimal,#label ").hide();	
		...
		case "mainnumDiv":
			$('.introjs-helperLayer').one('transitionend', function() {
				...
				$('#numDiv').fadeTo(100,1,function() {
			 		$('#intpart').fadeTo(100,1,function() {
						$('.introjs-tooltip').removeClass("hide");
						typingContent = 'First we convert the integral part <span class = color-yellow>12</span> and '+
										'then fractional part <span class = color-yellow>.625</span> to binary.'
		...
		case "intpart":
			...
			$('.introjs-helperLayer').one('transitionend', function() {
				$('#int1Div').fadeTo(1000,1,function() {
					...
					typingContent = '<ul><li>Take the integral value <span id = "int12Div">12</span>'+
									'</li><span id = step2></span>';
			...
			$('.introjs-helperLayer').one('transitionend', function() {
				...
				TweenMax.to('#fracpart',0.5,{opacity:1,top : 0,left:0, onComplete:function() {
					...
					typing('.panel2-text',typingContent,function() { 
						setTimeout(function() {
							typingContent = '<li>Take the fractional part from 12.<span style="font-weight:bold;">'+
									rightVal+'</span> mulitply it with 2.</li><span id = "two"></span>';
							typing('#one',typingContent,function() { 
								typingContent = 'We take the fractional part <span class = color-yellow>0.625</span> '+
												'and multiply it by <span class = color-yellow>2</span>.';
									typing('.introjs-tooltiptext',typingContent,function() { 
		...
		case "finalDivtext":
			...
			$('.introjs-helperLayer').one('transitionend', function() {
				$('#finalDivtext').append('<span id="floatText" class = "col-xs-offset-3 padding0">12.625 = '+
										'(<span id = "lef">1100</span>.<span id = "lef">101</span>)<sub>2</sub>'+
										'</span><br><span id = "floatText1" class = "marginleft30"></span>');
			typingContent = 'Now, as per <b>IEEE 754</b> standard specify the binary number into normalisation form.'+
							'<span id = "floatText2" class = "marginleft30"></span>'
			typing('#floatText1',typingContent,function() {
				typingContent = '<br>Normalisation form means before the decimal point we have only one value.'+
								'<span id = "floatText3" classs = "col-xs-offset-3 padding0"></span>'
		...
		case "shiftdotDiv":
			$('.introjs-helperLayer').one('transitionend', function() {
				...
				typing('.introjs-tooltiptext',typingContent,function() { 
					$('#shiftdotDiv').append('<div class = "col-xs-offset-3 col-xs-4 " id = "floatanimation">1<span '+
											'id = "manValDiv"><span id = "num1">&nbsp;&nbsp;1</span><span id = '+
											'"num2">&nbsp;&nbsp;0</span><span id ="num3">&nbsp;0</span><span id = '+
											'"num4">&nbsp;.</span>1&nbsp;0&nbsp;1</span>&nbsp;x <span id = "power3"> '+
											'2</span><sup id = exp3><sup>0</sup></sup></span></div>');
						...
						setTimeout(function() {
							...
							typing('#mulby2',typingContent,function() {
								...
								typingContent = '<ul><li>The <b>exponent</b>  is <span class = "opacity00" '+
												'id = exp3Div>3</span><span id = "binarybiasDiv"></span>';
								typing('#deduceDiv',typingContent,function() {
									...
									TweenMax.to('#exp3Div',2,{opacity:1,top : 0,left:0, onComplete:function() {
										$('.introjs-tooltipbuttons').append("<a style='background-color:green' "+
														"class='introjs-button animation-nextbtn'"+
														" onClick = 'animationbtn3()'>Next &#8594;</a>"); 
									}});	
								}); 
							}); 
						},3000);
			...
			$('.introjs-helperLayer').one('transitionend', function() {
				...
				typing('.introjs-tooltiptext',typingContent,function() {
					setTimeout(function() {
						...
						setTimeout(function() {
							$('#signval').text(0);
							typingContent = 'In Sign bit, we entered the value <span class="color-yellow">0</span> as '+
									'the sign of the given float value is positive(<span class="color-yellow">+ve</span>).';
							typing('.introjs-tooltiptext',typingContent,function() {
								$("#signval").effect( "highlight", {color:"#669966"}, 3000 );
								$('.introjs-tooltipbuttons').append("<a style='background-color:green' "+
													"class='introjs-button animation-nextbtn'>Next &#8594;</a>"); 
								$('.animation-nextbtn').click(function() { 
									...
									typing('.introjs-tooltiptext',typingContent,function() {
										...
										$('.animation-nextbtn').click(function() { 
											$('.animation-nextbtn').remove();
											var typingContent = 'The mantissa is <span class = color-yellow>100101</span>';
											typing('.introjs-tooltiptext', typingContent,function() {
												$('.introjs-tooltipbuttons').append("<a style='background-color:green' "+
															"class='introjs-button animation-nextbtn'>Next &#8594;</a>");
												$('.animation-nextbtn').remove();
	...
	introcode.start();
	$('.introjs-tooltipbuttons').append("<a  class='introjs-button' id='user-btn1'>&#8592; Back</a>");
	...
	$(".nextBtn,#divisionBtn,#user-btn1").hide();
...
function animationbtn3() {
	$('.animation-nextbtn').remove();	
	typingContent = '<br>We use <a href = "https://en.wikipedia.org/wiki/Exponent_bias">bias form</a> for 32 '+
					'bit and add 127 to exponent 3.<span id = "biasDiv"></span></li>';
	typing('#binarybiasDiv',typingContent,function() {
		$('#binarybiasDiv').append('<div><span id=demoMulBin2">Exponent = </span><span id = "demobytwo">'+
							'<span style="position:absolute; display:inline-block;" class = "opacity00" '+
							'id = "possibleExponent" >'+ $('#exp3').text()+'+'+127+
							'</span><span id=flipFront style="position : relative;" class = "opacity00">'
							+(parseInt($('#exp3').text())+127)+'</span></span></div>');
		 TweenMax.to("#possibleExponent",1,{rotationX:90,opacity:1, onComplete:function(){
			 TweenMax.fromTo("#flipFront",0.5,{rotationX:-90},{delay : 0.5, rotationX:0,opacity:1,onComplete:function(){
				...
				$('.animation-nextbtn').click(function() { 
					$('.animation-nextbtn').remove();
					$('#binarybiasDiv').append('<div><span id = "expdisplay" class = "opacity00">Exponent = '+
							'<span id=biasedExponent><span id=expToBin style="position:absolute;display:inline-block;" '+
							'class=opacity00>'+$('#flipFront').text() + '</span><span id=biasBinExponent style="position: relative;" '+
							'class=opacity00>'+(parseInt($('#flipFront').text())).toString(2) +'</span></span></div>');
					setTimeout(function() {
						...
						typing('.introjs-tooltiptext',typingContent,function() {
							...
							TweenMax.to('#expToBin',1,{rotationX:90,opacity:1, onComplete:function() {
								...
								$('.animation-nextbtn').click(function() { 
									...
									typing('.introjs-tooltiptext',typingContent,function() { 
										$('#biasedExponent').append('<br><span id = mantissaDiv>Mantissa = '+
														'<span id = mantissavalue>100101</span></span>');
...
function backfirststep() {
	 	divisionStep = 1;
		$(".user-btn").remove();
		$("#tableDiv, #desc").remove()
		introcode.goToStep(2);
}
...
function divisionClick(divisionStep) {
	...
	if (divisionStep == 1) {
		...
		TweenMax.to("#dividend",.5,{top:0,left:0,onComplete:function() {
			$("#Brace").fadeTo("slow","1",function() {
				...
				$('.introjs-tooltip').removeClass('hide');
				typingContent = 'In Binary System, since the base is <span class = color-yellow>2</span>, '+
								'we divide the integral value <span class = color-yellow>12</span> by '+
	...
	if (divisionStep == 3) {
		...
		$("#divisionBtn").hide();
		$(".user-btn").remove();
		typingContent = '<li>Take the quotient value i.e, '+$("#quotient").text()+'<span> divide it with 2.'+
						'</span></li><span id = "step3"></span>'
		typing('#step2',typingContent,function() { 
			...
			TweenMax.to("#dividend",.5,{top:0,left:0,onComplete:function() {
				...
				$("#multiplier").text(quotient * 2); 
				$('.introjs-tooltip').removeClass('hide');
				typingContent = 'In the next step, let us take the quotient of previous step as divisor and '+
								'divide it by <span class = color-yellow>2</span>.'
				typing('.introjs-tooltiptext',typingContent,function() {
					$("#divisor").fadeTo("slow","1",function() {
						TweenMax.to("#quotient",.5,{opacity:1,onComplete : function() {
							TweenMax.fromTo("#multiplier,#line",.5,{rotationX:-90},{rotationX:0,opacity:1,onComplete:function() {
	...
	if (divisionStep == 4) {
		...
		typing('#step3',typingContent,function() { 
			$('.introjs-tooltip').removeClass('hide');
			typingContent = 'Let us repeat the above steps i.e. divide the quotient by <span class = color-yellow>2'+
							'</span>, till we get the quotient as <span class = color-yellow>0</span>.';
			typing('.introjs-tooltiptext',typingContent,function() {
				setTimeout(function() {
					decimal();
				},1500);
	...
	if (divisionStep == 5) {
		...
		setTimeout(function() {
			...
			TweenMax.to("#fracBinSpan0",0.5,{delay:0.5,top:0,left:0,onComplete:function(){
				...
				typing('.introjs-tooltiptext',typingContent,function() {
					typingContent = '<li>Take the fractional part from <b>1.25</b> and multiply it(<b>.25</b>) '+
									'with 2.</li><span id = "three"></span>';
	...
	if (divisionStep == 6) {
		...
		TweenMax.to("#cpyMulFraction",.5,{opacity:1,top:0,left:0,onComplete:function() {
			$('.introjs-tooltip').removeClass('hide');
			typingContent = 'Let us take the fraction part <span class="color-yellow">.25</span> after '+
							'the decimal and multiply it with <span class="color-yellow">2</span>.';
			typing('.introjs-tooltiptext',typingContent,function() {
				rightDec = rightDec * 2;
				...
				TweenMax.fromTo("#mul2",.5,{y:-90},{opacity:1,y:0,onComplete:function() {
					...
					TweenMax.to("#fracBinSpan1",0.5,{delay:0.5,top:0,left:0,onComplete:function(){
						...
						$('.animation-nextbtn').click(function() { 
							$('.animation-nextbtn').remove();
							typingContent = 'Multiply the fractional part with <span class="color-yellow">2</span> '+
											'and take the integral part before the decimal and append it to the '+
											'right of the previous value.'
							typing('.introjs-tooltiptext',typingContent,function() {
								$("#divisionBtn").show();
							});
						});
	...
	if (divisionStep == 7) {
		...
		$('.introjs-tooltip').removeClass('hide');
		typingContent = 'Repeat the above steps i.e.multiply the fractional part with <span class="color-yellow">2</span>'+
						' and take the integral part before the decimal and append it to the right of the previous '+
						'value till we get the fractional part as <span class="color-yellow">0.0000</span>';
		typing('.introjs-tooltiptext',typingContent,function() {
	...
	if (divisionStep == 8) {
		...
		typing('#four',typingContent,function() {  
			...
			typingContent = '<span id = "fra">101</span> is the binary value for '+
							'<b>0.625</b>.';
			typing('#cpyMulFraction4',typingContent,function() { 
				$('.introjs-tooltip').removeClass('hide');
				typingContent = 'Let us combine the conversion of <span class="color-yellow">decimal-binary</span> ' + 
								'and <span class="color-yellow">fraction-binary</span> result.'
				typing('.introjs-tooltiptext',typingContent,function() {  
					$(".introjs-nextbutton, .introjs-prevbutton").show();
				});
			});
...
function division() {
	...
	$("#tableDiv").append("<table><tr id = tablechildDiv><td id=divisor>2</td id=Brace><td style=color:blue>)</td>"+
				"<td id=dividend>"+leftVal+"</td><td id=Brace style=color:blue>(</td><td id=quotient></td></tr>"+
				"<tr><td></td><td></td><td id=multiplier></td></tr><tr><td></td><td id=line colspan=3"+
				" style=color:blue>----------</td></tr><tr><td></td><td></td><td id=remainder></td></tr></table>");
...
function decimalDivision() {
	if ($("#dividend").text() >= 1) {
		...
		$("#divisionBtn").hide();
		$("#quotient").text('0');
		$("#multiplier").texdivisionClickt('0');
		$("#remainder").text('0');
	}
}
		...
		typing('#desc',typingContent,function() { 
			...
			typing('.introjs-tooltiptext',typingContent,function() {
				$('.introjs-tooltipbuttons').prepend("<a  class='introjs-button "+
						"user-btn' onclick='backfirststep()'>&#8592; Back</a>");
				$('.introjs-nextbutton').show();
			});
		});
	}
...
function myfunction1() {
	$(".nextBtn2, .nextBtn1").remove();
	$('.introjs-tooltip').addClass('hide');
	$('#floatdec').css({'opacity':'0'});
	$('#addDiv,.byte-div,#floatBox,#floatVar,#inputDiv').fadeTo(10,0)
	$('.introjs-tooltip').removeClass('hide');
	texttyping1();
}
...
function texttyping2() {
	typingContent = '<span class = color-yellow>Exponent</span> of the scientific notation is achieved by shifting the '+
			'<span class = color-yellow>radix</span> or decimal point to number of positions and adding with '+
			'<span class = color-yellow>127</span>. It occupies <span class = color-yellow>8</span> bits.</li>'
			typing('.introjs-tooltiptext',typingContent,function() {
				...
				$('.user-btn').click(function() {
					...
					$("#second2Div").css("opacity", 0);
					$("#secondDiv").css("opacity", 0);
					$("#third3Div").css("opacity", 0);
					$("#thirdDiv").css("opacity", 0);
					$('.introjs-tooltip').removeClass("hide");
					texttyping();
				});
				$('.animation-nextbtn').click(function() { 
...
function texttyping1() {
	typingContent = '<span class = color-yellow>Mantissa</span> is the right side of radix or decimal '+
			'point after exponent is achieved. It occupies <span class = color-yellow>23</span> bits.';
	typing('.introjs-tooltiptext',typingContent,function() {
...
function texttyping() {
	var typingContent = '<span class = "color-yellow">Sign bit</span> represents the sign of a given value.<br>'+
			'<ul><li>0 -- positive number</li><li>1 -- negative number</li>'
	typing('.introjs-tooltiptext',typingContent,function() {
		$('.introjs-tooltipbuttons').append("<a style='background-color:green' class='introjs-button animation-nextbtn' "+
				...
				$('#floatBox').fadeTo(1000,1,function() {
					$('.byte-div').fadeTo(1000,1,function() {
						$('#addDiv').fadeTo(1000,1,function() {
							...
							typing('.introjs-tooltiptext',typingContent,function() { 
							 	$('.introjs-tooltipbuttons').append("<a  class='introjs-button nextBtn2' onclick = 'myfunction1()'>&#8592; Back</a>");
