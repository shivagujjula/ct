		...
		(function($) {
			...
			Slick = (function() {
				...
				function Slick(element, settings) {
					...
					_.options = $
							.extend({}, _.defaults, settings, dataSettings);
			...
			Slick.prototype.applyTransition = function(slide) {
				...
				if (_.options.fade === false) {
					...
					transition[_.transitionType] = 'opacity ' + _.options.speed
			...
			Slick.prototype.autoPlayIterator = function() {
				...
				var _ = this, slideTo = _.currentSlide
				...
				if (_.options.arrows === true) {
					...
					if (_.slideCount > _.options.slidesToShow) {
						...
						if (_.options.infinite !== true) {
							_.$prevArrow.addClass('slick-disabled').attr(
									'aria-disabled', 'true');
						}
			...
			Slick.prototype.buildOut = function() {
				...
				_.$slides.each(function(index, element) {
					$(element).attr('data-slick-index', index).data(
				...
				_.$slideTrack = (_.slideCount === 0) ? $(
						...
						: _.$slides.wrapAll('<div class="slick-track"/>')
								.parent();
				...
				$('img[data-lazy]', _.$slider).not('[src]').addClass(
						'slick-loading');
				...
				_
						.setSlideClasses(typeof _.currentSlide === 'number' ? _.currentSlide
								: 0);
			...
			Slick.prototype.changeSlide = function(event, dontAnimate) {
				...
				case 'index':
					var index = event.data.index === 0 ? 0 : event.data.index
			...
			Slick.prototype.cleanUpEvents = function() {
				...
				if (_.options.dots && _.$dots !== null) {
					...
					$('li', _.$dots).off('click.slick', _.changeSlide).off(
							'mouseenter.slick', $.proxy(_.interrupt, _, true))
							.off('mouseleave.slick',
									$.proxy(_.interrupt, _, false));
				...
				$(window).off('load.slick.slick-' + _.instanceUid,
						_.setPosition);
				$(document).off('ready.slick.slick-' + _.instanceUid,
						_.setPosition);
			...
			Slick.prototype.getDotCount = function() {
				...
				if (_.options.infinite === true) {
					...
					pagerQty = 1 + Math
							.ceil((_.slideCount - _.options.slidesToShow)
									/ _.options.slidesToScroll);
				} else {
					while (breakPoint < _.slideCount) {
						++pagerQty;
			...
			Slick.prototype.getLeft = function(slideIndex) {
						...
						if (slideIndex + _.options.slidesToScroll > _.slideCount
							...
							if (slideIndex > _.slideCount) {
								_.slideOffset = ((_.options.slidesToShow - (slideIndex - _.slideCount)) * _.slideWidth)
										* -1;
								verticalOffset = ((_.options.slidesToShow - (slideIndex - _.slideCount)) * verticalHeight)
										* -1;
							} else {
								_.slideOffset = ((_.slideCount % _.options.slidesToScroll) * _.slideWidth)
										* -1;
				...
				if (_.options.variableWidth === true) {
					...
					if (_.options.rtl === true) {
						if (targetSlide[0]) {
							targetLeft = (_.$slideTrack.width()
									- targetSlide[0].offsetLeft - targetSlide
									.width())
									* -1;
						} else {
							targetLeft = 0;
					...
					if (_.options.centerMode === true) {
						...
						targetLeft += (_.$list.width() - targetSlide
								.outerWidth()) / 2;
					}
				}
							...
							.each(
									function(index, slide) {
										if (slide.offsetLeft - centerOffset
												+ ($(slide).outerWidth() / 2) > (_.swipeLeft * -1)) {
											swipedSlide = slide;
											return false;
										}
									});

					slidesTraversed = Math.abs($(swipedSlide).attr(
							'data-slick-index')
							- _.currentSlide) || 1;
			...
			Slick.prototype.initSlideEvents = function() {
				...
				if (_.options.pauseOnHover) {
					...
					_.$list.on('mouseenter.slick', $
							.proxy(_.interrupt, _, true));
					_.$list.on('mouseleave.slick', $.proxy(_.interrupt, _,
							false));
			...
			Slick.prototype.initializeEvents = function() {
				...
				$(document).on('ready.slick.slick-' + _.instanceUid,
						_.setPosition);
			...
			Slick.prototype.keyHandler = function(event) {
				...
				if (!event.target.tagName.match('TEXTAREA|INPUT|SELECT')) {
					if (event.keyCode === 37
						...
						_.changeSlide({
							data : {
								message : _.options.rtl === true ? 'previous'
										: 'next'
							}
						});
					}
			...
			Slick.prototype.lazyLoad = function() {
							...
							.each(
									function() {
										...
										var image = $(this), imageSource = $(
												this).attr('data-lazy'), imageToLoad = document
												.createElement('img');
				...
				loadRange = _.$slider.find('.slick-slide').slice(rangeStart,
						rangeEnd);
			...
			Slick.prototype.progressiveLazyLoad = function(tryCount) {
				...
				if ($imgsToLoad.length) {
					...
					imageToLoad.onload = function() {
						...
						image.attr('src', imageSource).removeAttr('data-lazy')
					...
					imageToLoad.onerror = function() {
						...
						if (tryCount < 3) {
							...
							/**
							 * try to load the image 3 times, leave a slight
							 * delay so we don't get servers blocking the
							 * request.
							 */
							setTimeout(function() {
							...
							_.$slider.trigger('lazyLoadError', [ _, image,
									imageSource ]);
			...
			Slick.prototype.registerBreakpoints = function() {
				...
				var _ = this, breakpoint, currentBreakpoint, l, responsiveSettings = _.options.responsive
						|| null;
			...
			Slick.prototype.removeSlide = Slick.prototype.slickRemove = function(
				...
				if (removeAll === true) {
					...
					_.$slideTrack.children(this.options.slide).eq(index)
							.remove();
				}
			...
			Slick.prototype.setCSS = function(position) {
				...
				x = _.positionProp == 'left' ? Math.ceil(position) + 'px'
						: '0px';
				y = _.positionProp == 'top' ? Math.ceil(position) + 'px'
						: '0px';
			...
			Slick.prototype.setDimensions = function() {
				...
				if (_.options.vertical === false
					...
					_.slideWidth = Math.ceil(_.listWidth
							/ _.options.slidesToShow);
					_.$slideTrack.width(Math.ceil((_.slideWidth * _.$slideTrack
			...
			Slick.prototype.setOption = Slick.prototype.slickSetOption = function() {
				 ...
				 *  - for changing a single option's value: .slick("setOption",
			...
			Slick.prototype.setSlideClasses = function(index) {
				...
				allSlides = _.$slider.find('.slick-slide').removeClass(
				...
				if (_.options.centerMode === true) {
					...
					if (_.options.infinite === true) {
						...
						if (index >= centerOffset
							...
							_.$slides.slice(index - centerOffset,
									index + centerOffset + 1).addClass(
									'slick-active')
							...
							allSlides.slice(indexOffset - centerOffset + 1,
									indexOffset + centerOffset + 2).addClass(
									'slick-active')
									.attr('aria-hidden', 'false');
						...
						if (index === 0) {
							...
							allSlides.eq(_.options.slidesToShow).addClass(
									'slick-center');
					...
					if (index >= 0
						...
						allSlides.addClass('slick-active').attr('aria-hidden',
								'false');
						...
						if (_.options.slidesToShow == _.options.slidesToScroll
							...
							allSlides
									.slice(
											...
											indexOffset + remainder).addClass(
					...
					if (_.slideCount > _.options.slidesToShow) {
						...
						for (i = _.slideCount; i > (_.slideCount - infiniteCount); i -= 1) {
							...
							$(_.$slides[slideIndex]).clone(true).attr('id', '')
									...
									.prependTo(_.$slideTrack).addClass(
											'slick-cloned');
						}
						for (i = 0; i < infiniteCount; i += 1) {
							...
							$(_.$slides[slideIndex]).clone(true).attr('id', '')
									.attr('data-slick-index',
											slideIndex + _.slideCount)
									.appendTo(_.$slideTrack).addClass(
											'slick-cloned');
						}
			...
			Slick.prototype.selectHandler = function(event) {
				...
				var targetElement = $(event.target).is('.slick-slide') ? $(event.target)
			...
			Slick.prototype.swipeEnd = function(event) {
				...
				if (_.touchObject.swipeLength >= _.touchObject.minSwipe) {
					...
					case 'down':
						...
						slideCount = _.options.swipeToSlide ? _
								.checkNavigable(_.currentSlide
										+ _.getSlideCount()) : _.currentSlide
								+ _.getSlideCount();
			...
			Slick.prototype.swipeHandler = function(event) {
				...
				_.touchObject.fingerCount = event.originalEvent
						&& event.originalEvent.touches !== undefined ? event.originalEvent.touches.length
						: 1;
			...
			Slick.prototype.swipeMove = function(event) {
				...
				_.touchObject.swipeLength = Math.round(Math.sqrt(Math.pow(
			...
			Slick.prototype.updateArrows = function() {
				...
				if (_.options.arrows === true
					...
					_.$prevArrow.removeClass('slick-disabled').attr(
							'aria-disabled', 'false');
					_.$nextArrow.removeClass('slick-disabled').attr(
							'aria-disabled', 'false');
					...
					if (_.currentSlide === 0) {
						...
						_.$nextArrow.addClass('slick-disabled').attr(
								'aria-disabled', 'true');
						_.$prevArrow.removeClass('slick-disabled').attr(
								'aria-disabled', 'false');
			...
			Slick.prototype.updateDots = function() {
				...
				if (_.$dots !== null) {
					...
					_.$dots.find('li').removeClass('slick-active').attr(
							'aria-hidden', 'true');

					_.$dots.find('li').eq(
							Math.floor(_.currentSlide
									/ _.options.slidesToScroll)).addClass(
			...
			$.fn.slick = function() {
				var _ = this, opt = arguments[0], args = Array.prototype.slice
						.call(arguments, 1), l = _.length, i, ret;
				for (i = 0; i < l; i++) {
