...
function ArduinoAvrCccReady() {
	introGuide();
	svgAppend("#outSideDiv", "svg");
	svgMarkerAppend("#svg", "arrow", "red");
}
...
function introGuide() {
	...
	introjs.onafterchange(function(targetElement) {
		...
		switch(elementId) {
			...
			case "heading":
				var text = "Let us learn the steps involved in writing and executing a <b class='ct-code-b-yellow'>Arduino</b> program.";
				...
				if (introjs._currentStep == 1) {
					$('.introjs-helperLayer').one('transitionend', function() {
						...
						typing('#step1Text', text, function() {
							$("#sourceCode").fadeTo(1000, 1, function() {
								...
								$(".popover").css({"top" : "0px;"});
								var text = "We can use a text editor to write a <b class='ct-code-b-yellow one'>Arduino</b> program.";
					...
					$('.introjs-helperLayer').one('transitionend', function() {
						...
						typing('#step2Text', text, function() {
							...
							var text = "The file name in which the <b class='ct-code-b-yellow two'>Arduino</b> code is written should" 
										+ " end with <span class='ct-code-b-yellow two'>.ino</span> extension.";
					...
					$('.introjs-helperLayer').one('transitionend', function() {
						...
						typing('#step3Text', text, function() {
							...
							var text = "Compilation has two main steps<br>1 - Object file creation and <br>2 - Linking<br>" 
										+ "<br>Compilation will generate <b class='ct-code-b-yellow three'>Hello.obj</b> file, "
...
function svgMarkerAppend(svgId, svgMarkerId, color) {
...
function svgLineAppend(svgId, svgLineId, markerId, className, x1, y1, x2, y2) {
...
function svgAnimatingLineRightToLeft(parentSelector, selector1, selector2, svgId, svgLineId, markerId, className, callBackFunction) {
	var parentOffset = $(parentSelector).offset();
	var x1 = $(selector1).offset().left - parentOffset.left + $(selector1).outerWidth();
	var y1 = $(selector1).offset().top - parentOffset.top + $(selector1).outerHeight() / 2;
	var x2 = $(selector2).offset().left - parentOffset.left;
	var y2 = $(selector2).offset().top - parentOffset.top + $(selector2).outerHeight() / 2;
	svgLineAppend(svgId, svgLineId, markerId, className, x1, y1, x1, y1);
...
function svgAnimatingLineLeftToRight(parentSelector, selector1, selector2, svgId, svgLineId, markerId, className, callBackFunction) {
	var parentOffset = $(parentSelector).offset();
...
function svgAnimatingLineTopToBottom(parentSelector, selector1, selector2, svgId, svgLineId, markerId, className, callBackFunction) {
	var parentOffset = $(parentSelector).offset();
...
function svgAnimatingLineSelector1BottomSideToSelector2TopSide(parentSelector, selector1, selector2, svgId, svgLineId, markerId, className, callBackFunction) {
	var parentOffset = $(parentSelector).offset();
	var x1 = ($(selector1).offset().left - parentOffset.left + $(selector1).outerWidth() / 2) - 2;
	...
	$("#compilationAboveText").removeClass("opacity00").addClass("loading");
		...
		setTimeout(function() {
			...
			setTimeout(function() {
				...
				svgMarkerAppend("#svg", "arrow1", "red");
				svgAnimatingLineRightToLeft("#outSideDiv", "#compilationIcon", "#span2", "svg", "line1", "arrow1", "svg-line", function() {
					$("#arrow1").css({opacity: '0'});
					svgMarkerAppend("#svg", "arrow2", "red");
					svgAnimatingLineTopToBottom("#outSideDiv", "#span2", "#span1", "svg", "line2", "arrow2", "svg-line", function() {
						$("#arrow2").css({opacity: '0'});
						svgAnimatingLineLeftToRight("#outSideDiv", "#span1", "#sourceCode", "svg", "line3", "arrow", "svg-line", function() {
...
function linkingFile() {
	...
	introjs.refresh();
	$('.introjs-helperLayer').one('transitionend', function() {
		zoomingEffect("#systemLibrariesDiv", function() {
			
		
		$(".step4, #step4Text").removeClass("opacity00");
		var text = $("#step4Text").html();
		typing('#step4Text', text, function() {
			svgAppend("#linkingDiv", "svg1");
			svgMarkerAppend("#svg1", "arrow3", "seagreen");
			svgAnimatingLineSelector1BottomSideToSelector2TopSide("#linkingDiv", "#systemLibrariesText", "#span3", "#svg1", "line4", "arrow3", "svg-line-seagreen", function() {
				$("#arrow3").css({opacity : '0'});
				svgMarkerAppend("#svg1", "arrow4", "seagreen");
				svgAnimatingLineLeftToRight("#linkingDiv", "#span3", "#linkIcon", "#svg1", "line5", "arrow4", "svg-line-seagreen", function() {
					...
					var text = "<span class='ct-code-b-yellow four'>Linker</span> links <span class='ct-code-b-yellow four'>one</span>"
								...
								+ " <span class='ct-code-b-yellow four'>single executable file</span>.";
	...
	$('.introjs-helperLayer').one('transitionend', function() {
		...
		typing('#step5Text', text, function() {
			...
			$("#output").fadeTo(1000, 1, function() {
				setTimeout(function() {
					$("i").removeClass("faa-passing");
					$("#outputLeftArrow").removeClass("opacity00").addClass('faa-passing-infinite animated');
					$(".step5").popover({
						placement: 'left',
						html: true,
						trigger: 'focus',
