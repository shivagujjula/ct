...
var binaryNumber = "";
var number = "";
var bitwiseLeftShiftOperatorReady = function() {
	var flag = true;
	$("#firstNum, #shiftNum").on("keydown keyup", function(e) { // conditions to enter text
	...
	function introSteps() {	
		...
		intro.onbeforechange(function(targetElement) {
			...
			case "inputDiv":
				...
				$("#addOne2 span").remove();
				//$("#aSpan, #line, #addOne").addClass("opacity00");
			break;
			case "binaryValueDiv":
				$("#originalValue span").remove();
				$("#appendNum, #arrow1").empty();
		...
		intro.onafterchange(function(targetElement) {
				...
				$(".introjs-helperLayer ").one('transitionend', function() {
					...
					TweenMax.to("#shiftNum", 1, {"opacity" : "1", onComplete:function() {
						...
						var text = "Select a qualifier, data type and enter the values to understand how a " 
							+ "bitwise <span class='ct-code-b-yellow'>left shift</span> operator works.";
				...
				$(".introjs-helperLayer ").one('transitionend', function() {
					...
					typing(".introjs-tooltiptext", text, function() {
						TweenMax.to("#aSpan", 1, {"opacity" : "1", onComplete:function() {
							$("#values").append("<span id='appendNum'>" + $("#firstNum").val() + "&emsp;</span>");
			...
			case "animationDiv":
				...
				$(".introjs-helperLayer ").one('transitionend', function() {
					$('.introjs-tooltip').removeClass('hide');
					var text = "Let us see the <span class='ct-code-b-yellow'>left shift</span> operator in action.";
					typing(".introjs-tooltiptext", text, function() {
						$('#valueDiv').append('<span id = "full"><span class = "opacity00" id="number2">' + binaryNumber + '&emsp;</span><span class="opacity00" id="shift2"><<&emsp;</span><span class="opacity00" id="shiftVal2">' + parseInt($("#shiftNum").val()) + '</span></span>');
						if ($("#firstNum").val() >= 0) {
							...
							$("#originalValue").addClass('z-index');
							...
							$("#twosComplementResult").addClass('z-index');
						}
						TweenMax.to("#number2",1,{top:0,left:0, onComplete:function() {
							...
							$("#twosComplementResult").removeClass('z-index');
							$("#originalValue").removeClass('z-index');
							$("#shift1").addClass('z-index');
							TweenMax.to("#shift2",1,{top:0,left:0, onComplete:function() {
							var l1 = $("#shiftNum").offset();
							$("#shiftVal2").text(parseInt($("#shiftNum").val())).removeClass("opacity00").offset({
								"top":l1.top,
								"left":l1.left + $("#shiftNum").width()/2
							});
							$("#shift1").removeClass('z-index');
...
function twosComplement() {
	$(".nextButton").remove();
	intro.refresh();
	for (var i = 0; i < $("#originalValue > span").length; i++) {
...
function twosComplementWithoutOne(i) {
	if(i >= 0) {
		if (i == $('#originalValue span').length-1) {
			if ($("#complementValue span:last-child").text() == 0) {
				TweenMax.to("#twosResultSpan" + ($('#originalValue span').length-1), 1, {"opacity" : "1", onComplete:function() {
					$("#twosResultSpan" + ($('#originalValue span').length-1)).text("1");
				}});
			}
		} else {
			if ($("#result" + i).text() == 1) {
...
function matter() {
	...
	if (parseInt($("#shiftNum").val()) == 0) {
		...
		$('.introjs-tooltip').removeClass('hide');
		var text ="Since we have choosen to shift the bits by <span class='ct-code-b-yellow'>" + parseInt($("#shiftNum").val()) + "</span> positions" 
			+ " we are removing the <span class='ct-code-b-yellow'>left</span> most " 
			+ "<span class='ct-code-b-yellow'>" + parseInt($("#shiftNum").val()) + "</span> bits and we will be " 
			+ "shifting the remaining bits by <span class='ct-code-b-yellow'>" + parseInt($("#shiftNum").val()) + "</span> positions.";
		typing(".introjs-tooltiptext", text, function() {
			$(".introjs-tooltipbuttons").append("<a class='introjs-button nextButton' onclick = 'removeZeros();'>Next &#8594;</a>");
		});
	} 
}
...
function removeZeros() {
	$(".nextButton").remove();
	intro.refresh();
	for (i = 0;i < parseInt($("#shiftNum").val()); i++) {
		$("#tdBorder" + i).effect("highlight",{color: 'red'}, 1000)
			TweenMax.to(('#cell' + i), 1, {top: -100, opacity: 1});
	}
...
function movingElements() {
	...
	intro.refresh();
	var count = 0;
	var len = parseInt($("#shiftNum").val());
	while (parseInt($("#table tr td").length/2)-parseInt($("#shiftNum").val()) > count) {
		$('#cell' + count).empty();
		$('#cell' + count).css({"top" : "0"});
		$("#cell"+ count).css({'opacity' : '1'});
		var index = count + 1;
		var l1 = $("#cell" + len).offset();
	...
	var remove = parseInt($("#table tr td").length/2)-parseInt($("#shiftNum").val())
	for(var i = remove; i<= parseInt($("#table tr td").length/2); i++) {
		$('#cell' + i).css({"opacity":"0"});
		$('#cell' + i).text(0);
	}
	if ($("#firstNum").val() > 0) {
		$('.introjs-tooltip').removeClass('hide');
		var text = "The newly created <span class='ct-code-b-yellow'>" + parseInt($("#shiftNum").val()) + "</span>" 
			+ " vacant positions to the <span class='ct-code-b-yellow'>right</span> will be filled" 
			+ " with <span class='ct-code-b-yellow'>zeros</span>.";
		typing(".introjs-tooltiptext", text, function() {
		...
		$('.introjs-tooltip').removeClass('hide');
		var text="As the given number is a negative number, the left most bit will be <span class='ct-code-b-yellow'>1</span>."
		typing(".introjs-tooltiptext", text, function() {
			$(".nextButton").remove();
  			$("#table").append("<div id='arrow' class='arrow' align='center'><i class='fa fa-arrow-up'></i></div>");
			$(".introjs-tooltipbuttons").append("<a class='introjs-button nextButton' onclick = 'matter2();'>Next &#8594;</a>");
		});
	}
}
...
function addZeros() {
	$(".nextButton").remove();
	var shiftLength = parseInt($("#table tr td").length/2)-parseInt($("#shiftNum").val());
    TweenMax.fromTo($("table  tr:eq(1)  td:nth-child(n+"+(shiftLength+1)+") span"),0.5,{top:'-100px'},{opacity:1,top:'0px'});
	if ($("#firstNum").val() >= 0) {
		$('.introjs-nextbutton, .introjs-prevbutton').show();
	} else { 
		$('.introjs-tooltip').removeClass('hide');
		var text = "Now, let us reconvert the binary number in two's complement form to get a decimal number to arrive at the final result."
		typing(".introjs-tooltiptext", text, function() {		
...
function negativeNumberResult() {
	...
	for (var i = 0 ; i < $("#originalValue > span").text().length; i++) {
		$("#originalResult").append('<span class="opacity00" id="originalResult' + i + '">0</span><span>&nbsp;</span>');
	}
	for(var i = 0; i < $("#originalValue > span").text().length; i++) {
	...
	$('.introjs-tooltip').removeClass('hide');
	var text ="This is the value after performing <span class='ct-code-b-yellow'>left shift</span> operation.";
	typing(".introjs-tooltiptext", text, function() {	
...
function reComplement() {
	...
	typing(".introjs-tooltiptext", text, function() {	
		for (var i = $("#originalValue > span").length-1; i >= 0; i--) {
...
function secondLineAndOne() {
	$(".nextButton").remove();
	intro.refresh();
	var text ="Now add 1 to the one's complement value to get two's complement value.";
	typing(".introjs-tooltiptext", text, function() {	
