...
var arr1;
var t = 0;
var overloadingBinaryMinusAsFriendFunctionReady = function() {
	introGuide();
	tl = new TimelineLite();
...
function introGuide() {
	...
	introjs.onbeforechange(function(targetElement) {
		...
		case 's3Sub':
			...
			case "s3Sub1" :
				...
				$('#friendFunction, #frndFunSub').hide();
	...
	introjs.onafterchange(function(targetElement) {
			...
			case "s3Sub1" :
				...
				$('.introjs-helperLayer ').one('transitionend', function() {
					...
					var text = 	"Now find out the addition of two objects <span class='ct-code-b-yellow'>s1</span> "+
								...
								"It will call like <span class='ct-code-b-yellow'>s3 = operator -(s1, s2);</span>";
						typing(".introjs-tooltiptext",text, function() {
		...
		case 'frndFunSub':
			$('.introjs-helperLayer ').one('transitionend', function() {
				$(".introjs-tooltip").removeClass('hide');
				var text = "Here, <span class='ct-code-b-yellow'>s1</span>" +
						   " and <span class='ct-code-b-yellow'>s2</span> both are passed as " +
						   " an <span class='ct-code-b-yellow'>arguments</span>.";
				typing(".introjs-tooltiptext",text, function() {
					tl.to("#frndFunSub", 1, {opacity: 1, onComplete: function() {
						$('.introjs-prevbutton, .introjs-nextbutton').show();
					}});
				});
			});
...
function fadeInBounceEffectWithTimelineMax(selector1, selector2, callBackFunction) {
	...
	var val;
	$(selector1).parent().addClass("z-index1000000").effect( "highlight",{color: 'blue'}, 1000);
		var l1 = $(selector1).offset();
		var l2 = $(selector2).offset();
		var topLength = l1.top - l2.top;
		var leftLength = l1.left - l2.left;
